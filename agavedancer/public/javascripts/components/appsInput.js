...
const AppsInput=React.createClass({
	...
	componentWillReceiveProps: function(nextProps) {
		...
		if (reload === 'resubmit') {
			this.setState({textValue: nextProps.data.value.value, fileValue: ''});
		} else if (reload === 'default') {
			this.setState({textValue: nextProps.data.value.default, fileValue: ''});
		}
	},
	...
	handleDsStoreChange: function(dsStore) {
		...
		if (dsItemPath) {
			let datastore=setting.datastore[dsItemPath.type === '__exampleData__' ? setting.public_datastore_type : dsItemPath.type];
