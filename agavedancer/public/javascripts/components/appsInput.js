...
const AppsInput=React.createClass({
	...
	buildAgaveAppsInput: function() {
		...
		if (data.value.visible === false) {
			...
			let insertButton=this.state.count < data.semantics.maxCardinality ? <Button bsStyle='btn-info' onClick={this.handleInsertInput}><Glyphicon glyph='plus' /> Insert</Button> : undefined;
			let removeButton=this.state.count > 1 ? <Button bsStyle='btn-info' onClick={this.handleRemoveInput}><Glyphicon glyph='minus' /> Remove</Button> : undefined;
			markup=(
				<div>
