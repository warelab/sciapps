...
```
Note that the input url (for 'trait') starting with 'agave://sciapps.org', which is critical for speeding up data transfer. You can get this url through browsing DataStore, then public, then example_data on SciApps.org with any app form (e.g. [ConvertTraitID](https://www.sciapps.org/app_id/ConvertTraitID-0.0.0)), or browsing data.sciapps.org and change the url (e.g. from 'https://data.sciapps.org/example_data/gwas_raw/height.txt' to 'agave://sciapps.org/example_data/gwas_raw/height.txt'), or upload them with follownig command:
